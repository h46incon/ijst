...
struct DeserFlag {
	...
	/**
	 * @brief  Set if move unknown (copy by default) from intermediate document when deserialize.
	 *
	 ...
	 */
	static const Flag kMoveTempDocUnknown		= 0x00000008;
};
...
public:
					...
					rapidjson::Document *pErrDocOut = IJST_NULL)
	{
		...
		if (IsBitSet(deserFlag, DeserFlag::kMoveTempDocUnknown)) {
			...
			IJSTI_PARSE_AND_RET_WHEN_ERROR;
			return DoFromJsonWrap<const JsonValue>(&Accessor::DoFromJson, doc, deserFlag, pErrDocOut);
		}
	}
	 ...
	 * @note	Handler allocator of nested object carefully. see DeserFlag::kMoveTempDocUnknown.
	 * @note 	It will not copy const string reference in source json. Be careful if handler such situation,
	 * 			e.g, json object is generated by ParseInsitu().
	 *
	 * @see DeserFlag::kMoveTempDocUnknown
	 */
	 ...
	 *
	 * @see DeserFlag::kMoveTempDocUnknown, MoveFromJson
	 */
