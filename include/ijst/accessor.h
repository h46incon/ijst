...
private:
	...
	const OverrideMetaInfos::MetaInfo* GetOvrMeta(size_t index) const
	{
		if (m_r->pOvrMetas == NULL) {
			return NULL;
		}
		assert(index < m_r->pOvrMetas->filedSize);
		return &(m_r->pOvrMetas->metaInfos[index]);
	}
